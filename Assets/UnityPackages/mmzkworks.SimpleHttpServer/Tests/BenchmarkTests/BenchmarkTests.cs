...
namespace mmzkworks.SimpleHttpServer.Tests
{
	public sealed class BenchmarkTests
	{
		...
		public IEnumerator Ping_Benchmark_100x_10cc_Succeeds() => UniTask.ToCoroutine(async () =>
		{
			...
			UnityEngine.Debug.Log($"[Benchmark] requests={requestCount}, concurrency={concurrency}, success={succeeded}, fail={failed}, min={min}ms, median={median:F2}ms, avg={avg:F2}ms, max={max}ms");
